...
The dependency graph across all four pillars. The achievement-web philosophy turned into a structural model.
...
- Boss defeats (signature drop = Medium; recruit join = Medium for map boss, Major for realm-final)
- Region objectives banked (per-objective unlocks: essence, qi, materials, passive)
- Region conquered (all 6 banked) = Medium
...
5. **Region conquest + objectives → cards added to pool → evolution + path discovery opportunities.** (Region objectives are load-bearing for passive unlocks.)
...
8. **Meridian refinement → path tier accessible → 5-card culmination recipe achievable.**
...
- **Build-tinkerer** — best-served. With rarity cut and paths being recipe-discovery puzzles, the build-tinkerer has 12 path arcs × 5 tiers = 60 discovery moments to chase.
