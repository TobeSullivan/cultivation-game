...
- **Asset ask:** Backdrop layer stack (mountains layer Ludo-generated, clouds layer Ludo-generated, mist hand-authored in Godot, sky Godot-native gradient), 10 individual building sprites × 2 visual stages = 20 base building images (head-only single-state status unresolved — see Open Questions), crowd FG silhouette pool (9 figures locked session 2), modal UI chrome.
...
- **Building visual style register under front-facing skyline (NEW 2026-05-23 session 2):** Backdrop landscape layers locked at flat vector (Ludo's reliable mode on landscape work, accepted tool-reality split). Architecture pieces locked painterly during session 1's iso compound work. Open question: do front-facing building sprites come back painterly the way the iso compound did, or do they flatten toward the backdrop style when the architecture is simpler and more vertical? Resolution: generate first building (Main Hall) using Reference 1 + a locked backdrop layer as Reference 2, evaluate style register, adjust if needed.
