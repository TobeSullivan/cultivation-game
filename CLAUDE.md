...
│   └── buildings/
│       └── main_hall.png    # locked 2026-05-23 session 3, true RGBA
├── game/                    # Godot project (created next session)
...
- **Art style (Locked 2026-05-23, updated 2026-05-23 session 2, confirmed session 3):** Hand-painted illustrated 2D was the original aspiration; tool reality split it. Camera/perspective: front-facing 2D skyline (not 3/4 angled top-down). Architecture pieces and the locked Main Hall (session 3) come back flat vector with deep-blue + red palette. Backdrop landscape layers also flat vector. Both layers harmonize as a unified flat-vector composition rather than fighting each other. Painterly aspiration retired without regret — flat vector is the locked style for the prototype. Locked in [asset-inventory.md > Style Locks](docs/prototype/asset-inventory.md).
...
- **Main Hall locked (2026-05-23 session 3):** First building generation. Canonical asset at `assets/buildings/main_hall.png` — true RGBA (1160×572), keyed via the three-step Generate-then-Edit-then-key pipeline. Reference 2 anchor for the buildings family — all 9 remaining buildings use locked Main Hall as Reference 2. Style register resolved (flat vector, deep-blue + red, harmonizes with backdrop). Building base includes baked platform / stairs / balustrade per drift pattern #18 — each building sprite carries its own ground edge. Compositing preview this session (Main Hall over mountains backdrop) confirmed style cohesion between the building and the locked mountains layer.
...
6. **Claude Code kick-off — NEXT SESSION.** Engine work begins. Godot project gets created in `game/`. First milestone: drop locked layers (sky / mountains / clouds / mist authoring / Main Hall) into a scene at real resolution. Author the platform horizon line. Drop a sample crowd FG figure (after running it through key_out_checkerboard.py) to validate the three-layer-with-depth read. The other 9 buildings continue in parallel using the locked three-step pipeline.
...
- **Building visual style register under front-facing skyline — ✅ RESOLVED 2026-05-23 session 3.** Main Hall came back flat vector (Ludo's reliable mode), composite with locked flat-vector backdrop confirmed style cohesion. Painterly-vs-vector concern dismissed. All 10 buildings will be flat vector. Open question retired.
