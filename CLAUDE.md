...
- **Art style (Locked 2026-05-23, updated 2026-05-23 session 2):** Hand-painted illustrated 2D, Avatar TLA aesthetic family — painterly cel-shading, visible brushwork, painted background washes. **Camera/perspective revised session 2:** front-facing 2D skyline (not 3/4 angled top-down). The iso/top-down framing was abandoned after the architecture pivot — Ludo holds painted style reliably on architecture pieces but drifts to flat vector on landscape-only work. Accepted tool-reality split: architecture pieces render painterly (TLA family), backdrop landscape layers render flat vector with hard shape gradients. Both styles coexist in the hub composition because they sit at different depth layers and the eye reads them as distinct planes. Locked in [asset-inventory.md > Style Locks](docs/prototype/asset-inventory.md).
- **Hub asset architecture (Locked 2026-05-23, redesigned 2026-05-23 session 2):** Front-facing skyline composition with separable parallax layers, composited at runtime in Godot. Layer stack: (1) sky — solid color or gradient in Godot, no Ludo asset; (2) mountains — Ludo-generated back layer, static or slow parallax; (3) clouds — Ludo-generated, horizontal drift; (4) mist — Godot-native hand-authored sprites, additive blend, slow drift; (5) buildings — 10 individual sprites with ground edges baked into base, foreground focus; (6) crowd FG — silhouette layer of disciples facing the sect, sparse bob/shuffle motion, scales with sect population. Mist and sky require no Ludo gens. The original iso compound architecture (background plate + 10 iso building sprites) was abandoned 2026-05-23 session 2 after the perspective mismatch open question and Ludo style-consistency failures on iso compound work. Drift pattern #18 (Population overlay axis) still respected — buildings remain separable sprites with anchor points; population is now an aggregate FG crowd density rather than per-building NPCs. Detail in [design-pass.md > Hub asset architecture](docs/prototype/design-pass.md).
...
- **Building visual style under front-facing skyline (NEW 2026-05-23 session 2) — OPEN.** Backdrop layers locked at flat vector (Ludo's reliable mode on landscape work). Architecture pieces locked painterly during session 1's iso compound work. Open question: do front-facing building sprites come back painterly the way the iso compound did, or do they flatten toward the backdrop style when the architecture is simpler and more vertical? Resolution: generate first building (Main Hall) using the locked Reference 1 + the locked backdrop (or a relevant subset) as Reference 2, evaluate style register, adjust if needed.

- **Head-only building tier states (PRE-EXISTING DISCREPANCY surfaced 2026-05-23) — OPEN, reframed under skyline architecture.** Original framing: design-pass.md Surface 5 specs "10 buildings × 2 visual stages = 20" but Art Inventory lists Main Hall + Personal Sanctum as "single state each" (= 18 total). Skyline architecture pivot doesn't resolve this — Main Hall and Personal Sanctum still need to be defined as either tier-progressing (3 stages each = +2 per building) or static (single state each). Decision pending. Lower priority than building style register since both head-only buildings still exist in the skyline.
